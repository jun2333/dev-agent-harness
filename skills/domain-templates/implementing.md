...
2. **遵循 TDD（测试先行，Red→Green）**：对每个可写测试的变更单元：
   a. **先写测试**（描述预期行为/边界，运行确认失败——Red）
   b. **再实现功能代码**（让测试通过——Green）
   c. **运行测试确认通过**后再进入下一个变更单元
   - 纯函数/工具逻辑（筛选、解析、格式化等）**必须测试先行**
   - 组件/路由以可测逻辑为界：可抽纯函数的部分先测试，UI 交互以 e2e/人工验证补充
3. 每完成一个步骤，验证是否达到预期
4. 如遇到问题，记录并评估影响
5. 所有步骤完成后，运行全量测试确认无回归，生成变更清单
6. 更新 Context Ledger（如适用）
