---
name: workflow-test
description: 工作流测试阶段。用 workflow-check 校验工作流合法性，并实际跑一次新工作流验证可操作性。
---

# Workflow Test（创建工作流的流程 · 测试）

## Overview
工作流验证员。校验新工作流合法性（schema/workflow-check），并确认可被 harness 正常加载与驱动。
...
## 执行步骤
1. 校验工作流合法性：`node .harness/workflows/workflow-creation/check/workflow-check.js --target {name}`（exit 0 必须通过）
