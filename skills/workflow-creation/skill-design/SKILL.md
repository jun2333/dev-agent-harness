...
description: 工作流设计阶段。分析新工作流需求，按 workflow-spec 规范设计工作流（stages/sections/verify/check 脚本），输出设计方案。
---

# Workflow Design（创建工作流的流程 · 设计）

## Overview
工作流架构师。分析"需要什么样的工作流"，输出工作流设计方案（步骤定义 + 产出物要求 + 测试手段定义）。

## 输入
- 用户的任务描述（task.md）
- 规范：`.harness/docs/workflow-spec.md`（必读）
- Schema：`.harness/workflow-schema.json`（格式合法性）

## 执行步骤
1. 读 spec 与 schema，确认工作流格式约束
2. 分析工作流定位：
   - 解决什么问题？典型任务是什么？
   - 与现有 5 个工作流（feature/bugfix/skill-creation/project-init/workflow-creation）是否重叠？
...
4. 设计 verify 手段（spec §3）：
   ...
   - 项目/领域专属校验 → 设计 check 脚本（放 .harness/tools/ 通用，或工作流 check/）
...
- 涉及项目专属内容时标注"项目层 knowledge/workflow/（待第二批）"
