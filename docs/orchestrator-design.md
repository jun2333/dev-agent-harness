...
1. **文件定义写死**：插件包（workflow.yaml）、项目配置（verify/env-check config.json）、任务清单（task.manifest.json）
...
| 命令 | 参数来源（禁止 LLM 生成） |
|---|---|
| `start` | 读 `task.manifest.json`（启动会话确认后由主 agent 写死） |
...
这是 harness「他证原则」的纲领化（verify.js 拒 `--commands`、env-check 拒自选命令的推广）。LLM 唯一涉及参数的点是启动会话（判断工作流 + 起 task-id），且已被 AskUserQuestion 用户确认 + 落盘 manifest 双重约束。
...
2. **上下文受控**：每阶段干净子代理 + 产出物摘要传递，读取范围程序化受控（详见 §7.1）；已读清单为辅助
3. **校验机械性**：子代理返回 JSON 摘要，schema 当场校验（sections_ok 必须 true）
4. **宿主可移植**：核心层零宿主依赖，适配层接口化，先落 CodeBuddy，预留其他宿主

### 编排器的定位：通用工作流执行引擎

编排器**不假设任务类型**——它只做一件事：读插件包（流程 + 测试手段），按阶段可靠执行。

按 harness 工作流插件规范（`docs/workflow-plugin-spec.md`），任何"流程 + 测试手段"的组合都能成为可编排的任务，**不限于代码**：

| 场景 | 插件包示例 | 测试手段（verify.checks） |
|---|---|---|
| 代码开发 | feature / bugfix / refactor | 命令池 key：unit / lint / e2e |
| 框架自举 | skill-creation / workflow-creation | 插件包 check/ 脚本：skill-check / workflow-check |
| 用户自定义 | 按规范写 `workflows/{name}/`（或项目层 `knowledge/plugins/{name}/`） | 任意：插件包 check/ 脚本 或 项目命令池 key |

这正是"工作流插件化"的设计意图：**用户按规范定制自己的 AI 工作流（流程 + 验收 + 测试手段），编排器负责让它被可靠执行**——写新技能、写工作流插件包、做文档/方案/调研等非代码任务同样适用。
...
| 文件读写 | Read / Write / Edit / Glob / Grep | 阶段产物、已读清单、checkpoint |
...
用户下达自然语言任务
  │
  ▼
[主 agent] 语义判断任务类型 → 给出候选工作流（feature/bugfix/refactor/skill-creation/...）
  │
  ▼
[主 agent] AskUserQuestion 一次问齐三件事：
  1. 工作流确认：展示 LLM 的判断 + 候选列表，用户选择/纠正
  2. task-id：用户输入或确认 LLM 生成的 kebab-case 标识
  3. task-desc：用户输入一句话任务描述
  │
  ▼
[主 agent] workspace 目录名 = "{task-id}-{desc}"（kebab-case，语义化）
  例：login-feature、bugfix-task-board-filter、skill-create-env-check
  │
  ▼
[主 agent] 写死 task.manifest.json（workflow / task-id / task-desc，参数落盘）
  │
  ▼
[程序] node orchestrator.js start --task-id <dir-name>
  │      （--workflow 从 manifest 读，LLM 不传）
```

规则：
- **工作流选择必须经用户确认**——LLM 的候选作为默认推荐，用户可改（这是"任务类型 → 工作流"的唯一 LLM 决策点，且不落单）
- **task-id + task-desc 用户提供**，编排器不自己生成
- workspace 目录名 = `{task-id}-{desc}`，**编排器内部流转只认这个目录名**（checkpoint / next / validate / advance 全部用它做 task-id）
- 目录名要求：全小写 kebab-case（`[a-z0-9-]`），不含空格/中文（中文描述不进入目录名，仅存在 task-desc 里供人读）
- **启动参数落盘**：主 agent 把确认结果写入 `workspace/{dir-name}/task.manifest.json`，start 只读文件，LLM 不拼 `--workflow`
...
start    --task-id <id>
         ...
         经 tools/workflow-lib.js 加载插件包（项目层优先 → 通用层兜底，schema 校验），
...
advance  --task-id <id> [--approved true]
         ...
         写 checkpoint、刷新已读清单、处理 on_fail 回退，输出下一阶段指令
...
**headless 形态的实现要点**：
- **spawn 命令**：`codebuddy -p "<prompt>" -y`（-y 跳过权限；按宿主 CLI 支持收窄工具集）。prompt = 阶段指令 + 上游产出物 + 已读清单 + **"最后输出 ```json {...}```"的 JSON 约定**
...
**层 2：规则层（schema 驱动，从插件包 stage 定义读）**

校验规则全部来自 `workflow.yaml` 插件包定义，不硬编码：
...
新插件包 / 新阶段 / 用户自定义工作流 → validate 自动适配，不改代码。
...
workflow.yaml ──解析──▶ checkpoint.json ──next──▶ 阶段指令 ──Agent prompt──▶ 子代理
                                        ...
                                        └─────────────通过───────────────────┘
                                                          │
                                            checkpoint.json ──advance──▶ context-snapshot → 已读清单
                                                                              │
                                              注入下一阶段 prompt ◀────────────┘
```

- **checkpoint.json**：编排器维护的唯一状态源（workflow / stage_outputs / on_fail 回退计数 / ctx_baseline）
- **阶段产物 md**：子代理写入 `workspace/{task-id}/`，既是证据也是下游输入
- **已读清单**：编排器模式下为**辅助**（见 §7.1 上下文控制策略）；跨阶段防重读主要靠产出物传递

### 7.1 上下文控制策略（编排器模式，与 CLI 版本质不同）

**CLI 版（单会话流）**：上下文**累积膨胀**——agent 读了什么都在对话里残留，所以上下文控制的目标是"防膨胀省 token"（tool-actions.log 记账 → 已读清单 → 防重读）。

**编排器版**：每阶段是**干净子代理**——读的文件只在子代理上下文里，阶段结束即消失，**不存在累积膨胀**。所以上下文控制的目标变成"**控制每个子代理的读取范围**"（只读需要的），而不是防膨胀。

编排器下上下文控制的三支柱（按可靠性排序）：

| 支柱 | 机制 | 可靠性 | 防什么 |
|---|---|---|---|
| 1. **产出物摘要传递** | 阶段 prompt 只给 skill + 上游产出物（design.md/task-plan.md）+ 已读清单 | 程序化，可靠 | 子代理读取范围本身（"prompt 给了什么"即限定） |
| 2. **干净子代理隔离** | 每阶段独立上下文，阶段结束即回收 | 架构天然 | 累积膨胀（根本不存在） |
| 3. **已读清单（只记主代理）** | 全局按日工具日志（`tool-actions/YYYY-MM-DD.log`，带 task_id），context-snapshot 按 task_id 过滤生成 | 确定性（主代理调用） | 不必要重读（省重复劳动，不是省上下文空间） |

**关键变化**：
- **子代理不做读取记账**：能读什么能写什么由任务 prompt + 交接文档限定（实测 2026-08-19 确认子代理的调用 hook 记录不到），不需要子代理自证——只记录主代理（桥）的工具调用
- **工具日志全局按日 + task_id**：`tool-actions/YYYY-MM-DD.log` 按日归档便于清理；每条带 `task_id`（来自 `.active-task.json` 全局任务状态，编排器 start 写 / done 清），context-snapshot 用脚本按 task_id 过滤（不耗 token）
- **防重读的可靠性让位于"产出物传递"**（架构天然、程序保证）——下游只消费上游摘要，本来就不需要读原始文件
- 已读清单从 CLI 版的"防膨胀防线"降级为编排器版的"防不必要重读的辅助"，只反映主代理执行
...
- 按日归档便于清理；context-snapshot 用脚本按 `task_id` 过滤生成已读清单（不耗 token）
...
| `context-snapshot.js` | 每阶段结束刷新已读清单，注入下一阶段 |
...
- 提取后校验逻辑**只有一份**，gate-check 与 validate 读同一份代码、同一份插件包定义，杜绝两套实现漂移
- stage 参数可注入（gate 从 checkpoint 读，validate 从命令行读），共享同一核心

### 8.1 与工作流插件化机制的对齐（关键）

编排器**复用 `tools/workflow-lib.js`** 作为插件包读取的单一入口，不自行解析 workflow.yaml——与 gate-check / verify.js 共享同一套插件包逻辑，杜绝"编排器一套解析、gate 一套解析"的漂移。

| 插件化能力 | 编排器怎么用 |
|---|---|
| 插件包发现（项目层 `knowledge/plugins/` 优先 → 通用层 `.harness/workflows/` 兜底） | `loadWorkflowDefinition(root, name)`——start 时加载，不自己找文件 |
| verify.checks 两级解析（插件包 `check/` 脚本 → 命令池 key） | `resolveVerifyCommands(root, wfDef)`——validate 校验 verify 证据时对账用 |
| 插件包 check/ 脚本（含项目层专属 `knowledge/plugins/{name}/check/`） | `findCheckScript(root, workflowName, check)`——子代理执行 verify 时调用 |
| workflow-schema.json 格式校验 | start 加载插件包后过 schema，非法定义拒绝启动 |
...
| 插件包自带 `templates/` 产出物模板 | prompt-builder 优先引用插件包 templates（项目层 → 通用层），fallback 到通用 `.harness/templates/` |

**对 gate 与 validate 的连带影响**：sections 的二维语义（外层组"任一满足"、内层"全部满足"）、require_verify、on_fail 都从插件包 stage 定义读取，validate 内嵌的 checkStage 逻辑必须复用 `loadWorkflowDefinition` 解析出的同一份 stage 对象，保证与 gate-check 行为一致。
...
│   │                        # 插件包读取复用 tools/workflow-lib.js（loadWorkflowDefinition /
│   │                        #   resolveVerifyCommands / findCheckScript），不自行解析 workflow.yaml
│   ├── prompt-builder.js    # 阶段指令组装（skill + 输入 + 输出 + 摘要规格 + 插件包 templates）
...
| 加固 | 已读清单注入、env-check 接入 start、收尾 gate-check 复核 | 全链路与现有机制打通 |
...
**背景**：编排器出现之前，harness 的流程执行靠"LLM 自读 workflow 自觉执行"（CLI 版）。编排器把流程状态机放进程序（阶段推进/校验/回退程序化），可靠性不依赖 LLM 自觉；插件化工作流的"流程即数据"只有在程序驱动的执行器下才有意义。但当前使用仍依赖 LLM 自觉选择（用户下达任务 → 主 agent 可走编排器，也可走 CLI 版）。
...
1. **【实测 2026-08-19】启动会话不完整——主 agent 桥可跳过 task-id/task-desc 确认**：跑 requirements 流程时，主 agent（桥）只向用户确认了工作流和范围，**擅自用推荐值写 manifest 的 task-id/task-desc，未让用户确认**（违背 orchestrate skill"AskUserQuestion 一次问齐三件事"的约定）。这是"主 agent 桥可靠性"的具体实例——桥没按 skill 完整执行启动会话。
   → 改进方向：启动会话**程序化强制**——`start` 校验 manifest 带用户确认标记（如 `user_confirmed: true`），缺失则拒绝启动并提示走完整启动会话；或新增 `preflight` 子命令把启动会话也变成脚本驱动（AskUserQuestion 由主 agent 执行但参数落盘校验）。
