...
│   ├── design-output.md           # 技术设计输出模板
│   ├── task-plan-output.md        # 任务计划输出模板
...
│       ├── skill-improvements-draft.md # 技能改进建议草稿（如有不足）
│       ├── checkpoint.json        # 状态检查点（断点恢复用）
│       └── execution-log.md       # 执行日志（对话历史）
│
└── harness.md                     # 主入口 prompt（启动 harness）
```
